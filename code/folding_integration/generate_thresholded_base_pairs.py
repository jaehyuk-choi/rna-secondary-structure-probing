...
def main():
    ...
    ap.add_argument('--no-threshold', action='store_true',
                    help='Write all pairs (no threshold filter), like unconstrained')
    args = ap.parse_args()

    use_threshold = not args.no_threshold

    # Collect sequence IDs for requested partitions
    partition_ids = {}
    with open(args.splits_csv) as f:
        for row in csv.DictReader(f):
            ...
            if p == 'NEW':
                ...
                continue
            if key not in partition_ids:
                partition_ids[key] = []
            partition_ids[key].append(row['id'])

    # Build union of IDs for requested partitions
    all_ids = []
    for p in args.partitions:
        key = 'new' if p.strip().lower() == 'new' else p.strip().upper()
    ...
    for model in args.models:
        ...
        seqs = all_ids[: args.limit] if args.limit else all_ids
        tasks = []
        for seq_id in seqs:
